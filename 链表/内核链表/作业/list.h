 ...
 */
#ifndef _LINUX_LIST_H   //假如没有定义 _LINUX_LIST_H 
#define _LINUX_LIST_H   //定义 _LINUX_LIST_H   作用防止头文件重复定义 
...
//初始化头节点  static: 修身函数为静态函数，只能在当前文件使用，防止与其他函数重名 
//inline:设置为内联函数，减少函数切换浪费的时间。 
...
/**
 * list_inser_loc  功能在loc位置之前插入一个节点
 * @loc : loc 用户需要插入的位置
 * @new : 需要插入的新节点
 * Note: if 'old' was empty, it will be overwritten.
 */
static inline void list_inser_loc(struct list_head *loc,struct list_head *xnew)
{
	  xnew->next = loc; 
	  xnew->prev = loc->prev; 
	  loc->prev->next = xnew; 
